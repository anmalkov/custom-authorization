...
- AuthorizationService will be executed for every request. This might significantly impact the performance of your application, especially if you are using a database or an external web service to store the user's roles. This scenario requires performance testing and a caching strategy.
- Due to criticality of the decisions made by AuthorizationService (allow or deny access), all business logic within this service must be fully (100%) tested.
- This service should provide logs and metrics that should be used for access auditing and threats detection.
- In case you use an external storage (such as a database or external web service) to store user's roles, that storage becomes a target for an attacker. By manipulating the data in this storage, an attacker can gain access to private information. That is why this storage should be part of a thorough security review.
